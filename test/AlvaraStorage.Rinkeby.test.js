...
contract('AlvaraStorage', function ([ owner, other ])
{
  ...
  it.only('Basic check that data of tokens different', async function ()
  {
    const COUNT = 10;
    const ITERATIONS = 10;
    const VALUE = this.TOKEN_PRICE.clone().imuln(COUNT);
    ...
    {
      await this.storage.mint(COUNT, { from: other, value: VALUE });
      console.log(i +1, 'iteration ready');
    }
    let all_data = [];
    for (let i = 0; i < COUNT * ITERATIONS; ++i)
    {
      const data = await this.storage.getData(i);
      console.log(i+1, '-th: ', data.toString());
      expect(all_data.includes(data.toString())).to.be.false;
      all_data.push(data.toString());
    }
  });
